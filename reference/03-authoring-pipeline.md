...
| **C** | Stage 6 (`What changed from the CrPC`) + Stage 4 (`Test your instinct`) | replace a range |
...
```js
const fs = require('fs');
const F = 'docs/chapters/chapter-13.md';
const l = fs.readFileSync(F, 'utf8').split('\n');
const chunk = fs.readFileSync('.tmp-s187a.md', 'utf8').replace(/\n$/, '').split('\n');

// Bound the search to this provision only.
const a = l.indexOf('## Section 187 — Procedure when investigation cannot be completed in twenty-four hours');
const end = l.indexOf('## Chapter consolidation — how long can they keep him?');
...
**Why the range bound matters.** `### The dispute this solves` appears once per provision. A bare `indexOf` finds the *first* one in the file and will happily splice chunk A for section 187 into section 173's block. The `a`/`end` bounds make that impossible.
...
```js
const i = l.findIndex((x, k) => k > a && k < end && x === '### What changed from the CrPC');
...
const sa = l.findIndex((x, k) => k > h5 && k < end && x.startsWith('**Arnesh Kumar v. State of Bihar**'));
const sb = l.findIndex((x, k) => k > sa && k < end && x.startsWith('**Joginder Kumar'));
...
Note the apostrophe rule for JS source: a needle containing an ASCII apostrophe (`### 6. The Advocate's Toolkit`) must sit in a **double-quoted** JS string.
...
const START = '> **187. Procedure when investigation cannot be completed in twenty-four hours.**';
...
const pairs = [
  ['shall forward the accused', '[[!shall forward the accused]]', 1],
  ['may from time to time authorise',  '[[?may from time to time authorise]]', 1],
...
**Constrained environment (no `npm`/`node` on PATH).** The CPC project ran with an ad-hoc codesigned Node binary at `.tooling/node`:
...
Set `DOCS_BASE` in the workflow to match the repository name.
...
- One commit per provision, with the provision named: `Migrate Sections 187 and 190 to the six-stage cognitive schema`.
