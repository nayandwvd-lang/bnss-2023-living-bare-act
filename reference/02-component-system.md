...
**VitePress local search indexes rendered markdown, not Vue component internals.** Any substantive text you move inside a `.vue` file silently vanishes from the search index. On a 500-section statute, search *is* the product.
...
export default defineConfig({
  markdown: { config: (md) => bnssContainers(md) }
})
```

| Container | Rendered class | Kicker / label | Used in |
|---|---|---|---|
| `oneminute` | `.bnss-oneminute` | "In one minute" | Stage 1 |
| `story` | `.bnss-story` | "Worked example · invented facts" | Stage 1 |
| `proviso` | `.bnss-proviso` | "Gating proviso" | Stage 2 |
| `instinct` | `.bnss-instinct` | "Test your instinct" | Stage 4 |
| `ruling` | `.bnss-ruling` | `<details>` / summary label | inside `instinct` |
...
::: oneminute The prosecution can lose the case before it opens the file.   ← right
```
...
```markdown
::::instinct The confession that was recorded correctly and still went out

Anita is arrested at 9 p.m. ... <facts> ...

Does the statement come in?
...
```js
md.inline.ruler.before('link', 'bnss_badge', fn)
```

| Syntax | Class | Tooltip title |
|---|---|---|
| `[[!shall be released]]` | `.bnss-mand` | Mandatory |
| `[[?may direct]]` | `.bnss-disc` | Discretionary |
| `[[=reason to believe]]` | `.bnss-art` | Term of art |
...
`resultKind` drives the rendered label: **"Then the court MUST"** vs **"Then the court MAY"**. Match it to the statute — a `may` section with a mandatory result is a substantive error, not a styling one.
...
"   →   “ ”      (quotation: “reason to believe”)
```

```
BROKEN:  { q: 'Is it the court\'s own record?' }
BROKEN:  { q: "Is it the court's own record?" }     ← breaks the outer HTML attribute
RIGHT:   { q: 'Is it the court’s own record?' }
```
...
`bnss-tree` counts high (hundreds) because the component emits the class on many child elements. Treat it as "non-zero", not as a count.
...
`docs/.vitepress/theme/custom.css` carries all the container, badge, tree and print styles under `.vp-doc .bnss-*` selectors. Copy it from `starter/` and adjust only the accent colour.

The CPC site uses `#8b1a1a` (deep red) as its theme colour. Pick something distinct for BNSS so the two sites are not confusable at a glance when both are open.
