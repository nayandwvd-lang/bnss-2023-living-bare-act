...
<template>
  <div class="bnss-tree" role="group" aria-label="Decision chain">
    <p class="bnss-tree-input">
      <span class="bnss-tree-inlabel">Input</span>
      <span>{{ props.input }}</span>
    </p>

    <ol class="bnss-tree-gates">
      <li v-for="(g, i) in props.gates" :key="i" class="bnss-tree-gate">
        <span class="bnss-tree-num">{{ i + 1 }}</span>
        <div class="bnss-tree-gbody">
          <p class="bnss-tree-q">{{ g.q }}</p>
          <p v-if="g.note" class="bnss-tree-note">{{ g.note }}</p>
          <p class="bnss-tree-no">
            <span class="bnss-tree-nolabel">If no</span>
            <span>{{ g.no }}</span>
          </p>
        </div>
      </li>
    ...
    <p class="bnss-tree-result" :class="`is-${props.resultKind}`">
      <span class="bnss-tree-rlabel">
        {{ props.resultKind === 'mandatory' ? 'Then the court MUST' : 'Then the court MAY' }}
      </span>
      <span>{{ props.result }}</span>
    </p>

    <p v-if="props.caveat" class="bnss-tree-caveat">
...
.bnss-tree-result.is-mandatory {
  background: var(--vp-c-danger-soft);
  border-color: var(--vp-c-danger-1);
}

.bnss-tree-result.is-mandatory .bnss-tree-rlabel {
  color: var(--vp-c-danger-1);
}

.bnss-tree-result.is-discretionary {
  background: var(--vp-c-tip-soft);
  border-color: var(--vp-c-tip-1);
}

.bnss-tree-result.is-discretionary .bnss-tree-rlabel {
