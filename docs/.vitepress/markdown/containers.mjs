...
function addBadges(md) {
  ...
  const KINDS = {
    '!': ['bnss-mand', 'Mandatory'],
    '?': ['bnss-disc', 'Discretionary'],
    '=': ['bnss-art', 'Term of art']
  }

  md.inline.ruler.before('link', 'bnss_badge', (state, silent) => {
...
export function bnssContainers(md) {
  ...
  addContainer(
    ...
    (tokens, idx) => {
      ...
      return (
        '<div class="bnss-oneminute">' +
        '<p class="bnss-oneminute-head">' +
        '<span class="bnss-oneminute-kicker">In one minute</span>' +
        (title ? `<span class="bnss-oneminute-title">${esc(title)}</span>` : '') +
        '</p>\n'
      )
    },
    () => '</div>\n'
  ...
  addContainer(
    ...
    (tokens, idx) => {
      ...
      return (
        '<div class="bnss-story">' +
        '<p class="bnss-story-head">' +
        '<span class="bnss-story-badge">Worked example · invented facts</span>' +
        (title ? `<span class="bnss-story-title">${esc(title)}</span>` : '') +
        '</p>\n'
      )
    },
    () => '</div>\n'
  ...
  addContainer(
    ...
    (tokens, idx) => {
      ...
      return (
        '<div class="bnss-proviso">' +
        '<p class="bnss-proviso-head">' +
        '<span class="bnss-proviso-kicker">Gating proviso</span>' +
        (title ? `<span class="bnss-proviso-title">${esc(title)}</span>` : '') +
        '</p>\n'
      )
    },
    () => '</div>\n'
  ...
  addContainer(
    ...
    (tokens, idx) => {
      ...
      return (
        '<div class="bnss-instinct">' +
        '<p class="bnss-instinct-head">' +
        '<span class="bnss-instinct-kicker">Test your instinct</span>' +
        (title ? `<span class="bnss-instinct-title">${esc(title)}</span>` : '') +
        '</p>\n'
      )
    },
    () => '</div>\n'
  ...
  addContainer(
    ...
    (tokens, idx) => {
      ...
      return (
        '<details class="bnss-ruling">' +
        `<summary><span>${esc(label)}</span></summary>` +
        '<div class="bnss-ruling-body">\n'
      )
    },
    () => '</div></details>\n'
  )
