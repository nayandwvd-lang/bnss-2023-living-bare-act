...
Everything here is live code lifted from the CPC project, with `cpc-` renamed to `bnss-` throughout. Copy it in and use it. Do not reimplement it — every one of these files encodes a decision that was made after something went wrong.
...
1. **`config.mjs` imports and calls `bnssContainers`.** Without the `markdown.config` hook, every `:::` block leaks into the page as literal text. This is the single most common setup failure.
2. **Class names agree.** `containers.mjs` emits `bnss-*`; `custom.css` styles `.vp-doc .bnss-*`; `MicroTree.vue` uses `bnss-tree*`. A stray `cpc-` anywhere means unstyled output that still builds cleanly — so it will not be caught by the build, only by looking at the page.
3. **`DOCS_BASE` in `deploy.yml` matches the repository name.** It is set to `/bnss/`. If the repo is called something else, change it, or every asset URL on the deployed site is wrong.
...
**`custom.css`** — carried over whole, so it still contains styles for CPC-era components that are not shipped in this kit (`ReadingProgress`, `LegalGlossary`, `SelfTest`, `KeyTerms`, `Flow`, `ViewMode`, `CivilSuitNavigator`). Those rules are inert. Prune them once the BNSS component set has settled; do not prune them on day one, because you may well want the same components.
